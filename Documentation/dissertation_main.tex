...
\title{Development of a\\Peer-to-Peer\\Messaging System\\Using\\Bluetooth\\on Mobile Devices }  % use \\ here otherwise you get a justified title
                                     % note capitalization of the title (only common 
                                     % words in lower case)
\author{Salah Ahmed Yasser\\Trouzine Mohamed\\Zaiz NourIslem Akram}            % your full name
\authorID{Q3551396}                   % your University Identifier
\supervisor{Prof.\ Alaa Eddine Belfedhal}           % your supervisor(s) name
\cosupervisor{Dr Alaa Eddine Belfedhal}                % your cosupervisor(s) name - no . in Dr *OPTIONAL* 
                                     % simply comment out the above line if absent
\department{Department of Computer Science}   % your department (e.g. Artificial Intelligence)
\faculty{Faculty of Science Exact}       % your faculty (e.g. ICT)
\degreeName{M.Sc.\ in Licence Degree}       % the degree you are reading
